...
#if defined(NDEBUG)
# error "Atlas cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x32c30e4501e52d01e89617589b49966c1f1a9ebae30425fa7ce6df1d8775d6d0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Atlas: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Atlas Signed Message:\n";
...
{
    // Atlas: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 250 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 12500000 * COIN;
            }

    // Subsidy is cut in half every 475000 blocks
    nSubsidy >>= (nHeight / 475000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Atlas: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Atlas: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1481620810, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "have a nice day";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 250 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048c3cb6f952cbbc1d73791fcd579ca0e574e46e91154cd6cce18ddaf630bf8b3e297c4c1a3728afcee13202d4a55b5e98baad4101b1574b20b7d8739537cd6eda") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1481620810;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2995379;
        ...
        assert(block.hashMerkleRoot == uint256("0xe67987eaa447372ce38b8ea4008788b5254658bfbe52eed889c88ce314ceb78c"));
...
unsigned char pchMessageStart[4] = { 0x2f, 0x64, 0x31, 0xc2 }; // Atlas: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AtlasMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AtlasMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AtlasMiner(CWallet *pwallet)
{
    printf("AtlasMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("atlas-miner");
    ...
    try { loop {
        ...
        printf("Running AtlasMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AtlasMiner, pwallet));
}
